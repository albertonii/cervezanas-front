...
export function SearchCheckboxCPMobiles({
    ...
    const handleCheckboxChange = (cp: ICPMobile, isChecked: boolean) => {
        if (isChecked) {
            ...
            // Eliminar de la lista de CP eliminados
            const removedCPsMobile = getValues('removed_cps_mobile');

            setValue(
                'removed_cps_mobile',
                removedCPsMobile.filter((item: any) => item.id !== cp.id),
            );
        } else {
            setCheckedCPsState(
            ...
            // Añadir a la lista de CP eliminados
            const removedCPsMobile = getValues('removed_cps_mobile');

            setValue('removed_cps_mobile', [
                ...removedCPsMobile,
                { id: cp.id },
            ]);
        }
