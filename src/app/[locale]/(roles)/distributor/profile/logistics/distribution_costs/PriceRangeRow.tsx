  ...
  return (
    <>
      <fieldset className="mr-2 flex flex-col gap-4 rounded-xl border p-2 sm:flex-row">
        <legend className=" text-gray-600">Franja de Precio {index + 1}</legend>
        ...
        <label className="">
          ...
          <input
            ...
            onChange={(event) => handleInputLowerChange(index, event)}
            placeholder="0"
            className={`
                ${
        ...
        <label className="">
          ...
          <input
            ...
            onChange={(event) => handleInputUpperChange(index, event)}
            placeholder="50"
            className={`
                ${
                  errors.distribution_range_cost &&
        ...
        <label className="">
          ...
          <input
            ...
            onChange={(event) => handleInputCostChange(index, event)}
            placeholder="30"
            className={`
                ${
                  errors.distribution_range_cost &&
      ...
      {errors.distribution_range_cost && errors.distribution_range_cost[index] && (
        <div className="flex flex-col gap-2">
          {errors.distribution_range_cost[index]?.lower && (
            <DisplayInputError
              message={errors.distribution_range_cost[index]?.lower?.message}
            />
          )}

          {errors.distribution_range_cost[index]?.upper && (
            <DisplayInputError
              message={errors.distribution_range_cost[index]?.upper?.message}
            />
          )}
          ...
          {errors.distribution_range_cost[index]?.shippingCost && (
            <DisplayInputError
              message={
                errors.distribution_range_cost[index]?.shippingCost?.message
              }
            />
          )}
          ...
          {errors.distribution_range_cost &&
            errors.distribution_range_cost[index]?.lower && (
              <DisplayInputError message={"errors.lower_greater_than_upper"} />
            )}
        </div>
      )}
    </>
